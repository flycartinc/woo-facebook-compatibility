echo "Woo facebook compatibility"
current_dir="$PWD"
composer_run(){
...
copy_folder(){
  ...
  compressed_plugin_folder=$pack_folder"/woo-facebook-compatibility"
  if [ -d "$pack_folder"]; then
    rm -r "$pack_folder"
  fi
  mkdir "$pack_folder"
  mkdir "$compressed_plugin_folder"
  move_dir=("App" "vendor" "woo-facebook-compatibility.php")
...
function zip_folder(){
  ...
  zip_name="woo-facebook-compatibility"
  rm "$zip_name".zip
